...
function post (apiUrl, data, cookieId) {
    return new Promise((resolve, reject) => {
        db.get('librevent').then(keypair => {
            ...
            xhr.setRequestHeader('X-trex-Version', config.VERSION);
            xhr.setRequestHeader('X-trex-Build', config.BUILD);
            ...
            xhr.setRequestHeader('X-trex-PublicKey', keypair.publicKey);
            xhr.setRequestHeader('X-trex-Signature', bs58.encode(signature));
...
function get (apiUrl, version, userId) {
    ...
    return new Promise((resolve, reject) => {
        db.get('librevent').then(keypair => {
            ...
            xhr.setRequestHeader('X-trex-Version', version);
            xhr.send();
            xhr.onload = function () {
                if (this.status >= 200 && this.status < 300) {
